...
the configured policy, signed precheck plan, canonical statement bindings, and
the complete guardrail bundle are evaluated before signing.

Each SQL migration embeds a canonical, generator-attested and release-signed
artifact. Its plan, check, and guardrail bundle digests are also bound into the
