...
operator, timestamps, from/to versions, superseded revisions, and reversal
link. `statement_attempts` and `events` retain immutable per-attempt evidence.

`Store.Status` is deliberately read-only. Callers must pass a manifest already
